    ...
    <div class="">
      ...
      <div class="all">
        ...
        <div class="commentBtnBox">
          <mt-button type="default" class="commentBtn" @click="$router.push({name:'actComments',query:{id}})">我要评论</mt-button>
        </div>
      </div>
    </div>
  </template>
    ...
    export default {
      ...
      methods: {
        ...
        loadMore() {
          this.pn = this.pn + 1
          this.loading = true
          this.$axios.get(`/jsp/wap/trActivity/ctrl/jsonCommentPage.jsp?id=${this.id}`, {
            params: {
              pageNumber: this.pn
            }
          }).then(res => {
